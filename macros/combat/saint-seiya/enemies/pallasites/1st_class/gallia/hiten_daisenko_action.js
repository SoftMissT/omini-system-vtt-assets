...
(async () => {
    ...
    ChatMessage.create({
        ...
        content: `
            <div style="background: #000; border: 2px solid gold; padding: 15px; color: #fff; font-family: 'Rajdhani';">
                <h2 style="color: gold; text-align: center; font-family: 'Orbitron'; letter-spacing: 2px;">Hiten Daisenko</h2>
                <div style="text-align: center; margin-bottom: 10px; font-size: 0.9em; opacity: 0.8;">Gallia</div>
                <p style="border-top: 1px solid #333; padding-top: 10px;">Flash de luz divina em área.</p>
                <div style="background: #222; padding: 5px; text-align: center; margin-top: 10px; border-radius: 4px;">
                    <strong>DANO:</strong> <span style="color: #ff2b4a;">10d6 Light</span>
                </div>
            </div>
        `
    });
    ...
    // Roll
    const dmg = "10d6 Light";
    if (dmg && dmg.includes("d")) {
        await OmniCore.dice.roll(dmg);
    }

    // VFX
    const isTime = "Flash de luz divina em área.".toLowerCase().includes("tempo") || "Flash de luz divina em área.".toLowerCase().includes("time");
