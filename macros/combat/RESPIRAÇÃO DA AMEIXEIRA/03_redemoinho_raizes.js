(async () => {
  ...
  const cfg = {
    nome: "Terceira Forma: Redemoinho de Raízes Duplas",
    jp: "San no Kata: Futae Ne no Uzumaki",
    tipo: "Ação Padrão",
    cor: "#A6535A",
    niveis: [
      { n: 1, d: "1d4", c: 3 },
  ...
  const lvl = await new Promise((r) => {
    new Dialog({
      title: "��� Área",
      content: `<div style="background:#0a0a0f;padding:15px;border:2px solid ${cfg.cor}"><h2 style="color:${cfg.cor}">${cfg.nome}</h2>${cfg.niveis.map((n) => `<p>Nv${n.n}: ${n.d} raio 3m | ${n.c}PC</p>`).join("")}</div>`,
      buttons: {
