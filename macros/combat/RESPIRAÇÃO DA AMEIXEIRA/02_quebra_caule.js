(async () => {
  ...
  const cfg = {
    nome: "Segunda Forma: Quebra do Caule",
    jp: "Ni no Kata: Kuki Kudaki",
    tipo: "Ação Padrão",
    cor: "#A6535A",
    niveis: [
      { n: 1, d: "1d6", mov: "1.5m", c: 2 },
      { n: 2, d: "1d8", mov: "3m", c: 3 },
      { n: 3, d: "1d10", mov: "4.5m", c: 4 },
      { n: 4, d: "2d8", mov: "ZERO", c: 6 },
    ],
  };
  const lvl = await new Promise((r) => {
    new Dialog({
      title: "���",
      content: `<div style="background:#0a0a0f;padding:15px;border:2px solid ${cfg.cor}"><h2 style="color:${cfg.cor}">${cfg.nome}</h2>${cfg.niveis.map((n) => `<p>Nv${n.n}: ${n.d} | Reduz ${n.mov} | ${n.c}PC</p>`).join("")}</div>`,
      buttons: {
  ...
  const chat = `<div style="background:linear-gradient(135deg,#0a0a0f,#1a1a2e);border:2px solid ${cfg.cor};border-radius:12px;padding:15px"><h2 style="color:${cfg.cor};text-align:center">��� ${cfg.nome}</h2><div style="background:rgba(166,83,90,0.1);padding:10px;text-align:center"><div style="font-size:32px;color:#FFD700;font-weight:bold">${roll.total}</div></div><div style="background:rgba(166,83,90,0.2);padding:8px;margin-top:10px"><strong>⚡</strong> Reduz movimento: ${sel.mov}<br><strong>��� Poda:</strong> Se alvo já reduzido, +1d4 dano</div></div>`;
