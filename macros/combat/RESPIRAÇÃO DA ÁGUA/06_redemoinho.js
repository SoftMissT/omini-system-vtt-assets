(async () => {
  ...
  const cfg = {
    nome: "Sexta Forma: Redemoinho",
    jp: "Roku no Kata: Nejire Uzu",
    tipo: "Ação Padrão",
    cor: "#048ABF",
    niveis: [
      { n: 1, d: "1d8", c: 4 },
  ...
  const lvl = await new Promise((r) => {
    new Dialog({
      title: "���",
      content: `<div style="background:#0a0a0f;padding:15px;border:2px solid ${cfg.cor}"><h2 style="color:${cfg.cor}">${cfg.nome}</h2>${cfg.niveis.map((n) => `<p>Nv${n.n}: ${n.d} | ${n.c}PC | Raio 3m</p>`).join("")}</div>`,
      buttons: {
