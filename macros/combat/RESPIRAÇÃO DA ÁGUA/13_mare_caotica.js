(async () => {
  ...
  const cfg = {
    nome: "Variação: Maré Crescente - Caótica",
    jp: "Uchishio - Ran",
    tipo: "Ação Bônus",
    cor: "#048ABF",
    niveis: [
      { n: 1, d: "2x1d4", acerto: 2, c: 2 },
      { n: 2, d: "2x1d6", acerto: 3, c: 3 },
      { n: 3, d: "2x1d8", acerto: 4, c: 4 },
      { n: 4, d: "2x1d10", acerto: 5, c: 5 },
    ],
  };
  const lvl = await new Promise((r) => {
    new Dialog({
      title: "���",
      content: `<div style="background:#0a0a0f;padding:15px;border:2px solid ${cfg.cor}"><h2 style="color:${cfg.cor}">${cfg.nome}</h2>${cfg.niveis.map((n) => `<p>Nv${n.n}: ${n.d} | +${n.acerto} acerto | ${n.c}PC</p>`).join("")}</div>`,
      buttons: {
