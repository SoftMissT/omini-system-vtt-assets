(async () => {
  ...
  const cfg = {
    nome: "Variação: Redemoinho - Correnteza",
    jp: "Nejire Uzu - Ryūryū",
    tipo: "Ação Bônus",
    cor: "#048ABF",
    niveis: [
      { n: 1, d: "1d6", redir: 1, bloq: 4, c: 3 },
      { n: 2, d: "1d8", redir: 1, bloq: 4, c: 4 },
      { n: 3, d: "1d10", redir: 2, bloq: 4, c: 5 },
      { n: 4, d: "2d6", redir: 2, bloq: 4, c: 6 },
    ],
  };
  const lvl = await new Promise((r) => {
    new Dialog({
      title: "���",
      content: `<div style="background:#0a0a0f;padding:15px;border:2px solid ${cfg.cor}"><h2 style="color:${cfg.cor}">${cfg.nome}</h2>${cfg.niveis.map((n) => `<p>Nv${n.n}: ${n.d} | Redir ${n.redir} proj | +${n.bloq} bloq | ${n.c}PC</p>`).join("")}</div>`,
      buttons: {
