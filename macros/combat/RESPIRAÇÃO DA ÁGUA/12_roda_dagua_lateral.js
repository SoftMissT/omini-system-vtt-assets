(async () => {
  ...
  const cfg = {
    nome: "Variação: Roda d'Água Lateral",
    jp: "Yoko Mizu Guruma",
    tipo: "Ação Bônus",
    cor: "#048ABF",
    niveis: [
      { n: 1, d: "1d6", c: 1, mom: 1 },
      { n: 2, d: "1d8", c: 2, mom: 1 },
      { n: 3, d: "1d10", c: 3, mom: 1 },
      { n: 4, d: "2d6", c: 4, mom: 1 },
    ],
  };
  const mom = actor.system.props?.momentum?.value || 0;
  if (mom < 1) return ui.notifications.error("❌ Requer 1 Momentum!");
  const lvl = await new Promise((r) => {
    new Dialog({
      title: "���",
      content: `<div style="background:#0a0a0f;padding:15px;border:2px solid ${cfg.cor}"><h2 style="color:${cfg.cor}">${cfg.nome}</h2>${cfg.niveis.map((n) => `<p>Nv${n.n}: ${n.d} linha | ${n.c}PC+${n.mom}Mom</p>`).join("")}</div>`,
      buttons: {
