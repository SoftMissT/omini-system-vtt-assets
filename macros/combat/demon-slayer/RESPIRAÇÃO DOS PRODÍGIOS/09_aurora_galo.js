...
const cfg={nome:"Nona Forma: Aurora Carmesim do Galo",jp:"Kyū no Kata — Niwatori no Asahi",tipo:"Padrão",cor:"#6F3A9C",animal:"��� GALO (Chicote)",niveis:[{n:1,d:"1d8",c:3},{n:2,d:"1d10",c:4},{n:3,d:"2d8",c:5},{n:4,d:"2d10",c:6}]};
const lvl=await new Promise(r=>{new Dialog({title:"⚡",content:`<div style="background:#0a0a0f;padding:15px;border:2px solid ${cfg.cor}"><h2 style="color:${cfg.cor}">${cfg.nome}</h2>${cfg.niveis.map(n=>`<p style="color:#fff">Nv${n.n}: ${n.d} | ${n.c}PC</p>`).join('')}<p style="color:#2EFF7A;font-size:12px">Remove 1 condição de aliado (6m)</p></div>`,buttons:{n1:{label:"Nv1",callback:()=>r(0)},n2:{label:"Nv2",callback:()=>r(1)},n3:{label:"Nv3",callback:()=>r(2)},n4:{label:"Nv4",callback:()=>r(3)}}}).render(!0)});
...
<div style="background:rgba(46,255,122,0.15);padding:8px;border-left:3px solid #2EFF7A;margin-top:10px">
  <strong style="color:#2EFF7A">✨ Remove 1 Condição Negativa</strong> de aliado (6m)<br>
  <strong style="color:#A855F7">${cfg.animal}</strong> — Aliados ganham +2 acerto vs marcados
</div>
...
ui.notifications.info(`��� Aurora | Dano: ${roll.total} | Condição removida!`);
})();
