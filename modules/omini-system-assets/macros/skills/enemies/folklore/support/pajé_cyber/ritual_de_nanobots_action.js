/* ═══════════════════════════════════════════════════════════════
 * FOLKLORE SKILL: Ritual de Nanobots
 * User: Pajé Cyber
 * ═══════════════════════════════════════════════════════════════ */
(async () => {
    ...
    ChatMessage.create({
        ...
        content: `
            <div style="background: url('modules/omini-system-assets/assets/bg/leather_texture.webp') repeat, #3e2723; border: 2px solid blue; padding: 15px; color: #fff8e1; font-family: 'Courier New', monospace;">
                <h2 style="color: blue; text-align: center; text-transform: uppercase; border-bottom: 2px dashed #8d6e63;">Ritual de Nanobots</h2>
                <div style="text-align: center; margin-bottom: 10px; font-size: 0.8em; color: #d7ccc8;">Pajé Cyber</div>
                <p style="padding-top: 10px; font-style: italic;">Cura e reparo de armaduras.</p>
                <div style="background: #5d4037; padding: 5px; text-align: center; margin-top: 10px; border-radius: 4px;">
                    <strong>EFEITO:</strong> <span style="color: #ffcc80;">Heal/Repair</span>
                </div>
            </div>
        `
    });
    ...
    if ("Ritual de Nanobots" === "Estrela de Sete Pontas") {
        ui.notifications.info("��� Disparando 7 punhais solares! Se 3 acertarem: Quebra de Armadura.");
    }

    // Roll
    const dmg = "Heal/Repair";
    if (dmg && dmg.includes("d")) {
        await OmniCore.dice.roll(dmg);
    }

    // VFX
    const isFire = "Cura e reparo de armaduras.".toLowerCase().includes("fogo") || "Cura e reparo de armaduras.".toLowerCase().includes("solar");
