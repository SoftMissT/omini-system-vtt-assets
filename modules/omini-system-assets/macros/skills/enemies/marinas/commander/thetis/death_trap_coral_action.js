...
(async () => {
    ...
    if ("Thetis" === "Io" && "Death Trap Coral" === "The Six Beasts") {
        new Dialog({
    ...
    ChatMessage.create({
        ...
        content: `
            <div style="background: linear-gradient(to bottom, #001a33, #003366); border: 2px solid coral; padding: 15px; color: #b3d9ff;">
                <h2 style="color: coral; text-align: center; font-family: 'Orbitron'; text-shadow: 0 0 5px #00ccff;">Death Trap Coral</h2>
                <div style="text-align: center; margin-bottom: 10px; font-size: 0.8em; color: #66a3ff;">Thetis - Marina General</div>
                <p style="border-top: 1px solid #004d99; padding-top: 10px; font-style: italic;">Armadilha de Coral. Calcificação.</p>
                <div style="background: #00264d; padding: 5px; text-align: center; margin-top: 10px; border: 1px solid #004d99;">
                    <strong>DANO/EFEITO:</strong> <span style="color: #33ccff;">3d6 + Calcify</span>
                </div>
            </div>
        `
    });
    ...
    // Roll Padrão
    const dmg = "3d6 + Calcify";
    if (dmg && dmg.includes("d") && "Death Trap Coral" !== "The Six Beasts") {
        await OmniCore.dice.roll(dmg);
    }
