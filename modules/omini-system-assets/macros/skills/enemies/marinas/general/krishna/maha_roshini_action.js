...
(async () => {
    ...
    if ("Krishna" === "Io" && "Maha Roshini" === "The Six Beasts") {
        new Dialog({
    ...
    ChatMessage.create({
        ...
        content: `
            <div style="background: linear-gradient(to bottom, #001a33, #003366); border: 2px solid orange; padding: 15px; color: #b3d9ff;">
                <h2 style="color: orange; text-align: center; font-family: 'Orbitron'; text-shadow: 0 0 5px #00ccff;">Maha Roshini</h2>
                <div style="text-align: center; margin-bottom: 10px; font-size: 0.8em; color: #66a3ff;">Krishna - Marina General</div>
                <p style="border-top: 1px solid #004d99; padding-top: 10px; font-style: italic;">Grande Luz. Barreira de Chakras.</p>
                <div style="background: #00264d; padding: 5px; text-align: center; margin-top: 10px; border: 1px solid #004d99;">
                    <strong>DANO/EFEITO:</strong> <span style="color: #33ccff;">Immunity</span>
                </div>
            </div>
        `
    });
    ...
    // Roll Padrão
    const dmg = "Immunity";
    if (dmg && dmg.includes("d") && "Maha Roshini" !== "The Six Beasts") {
        await OmniCore.dice.roll(dmg);
    }
