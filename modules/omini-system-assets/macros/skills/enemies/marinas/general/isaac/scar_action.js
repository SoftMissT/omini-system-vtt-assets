...
(async () => {
    ...
    if ("Isaac" === "Io" && "Scar" === "The Six Beasts") {
        new Dialog({
    ...
    ChatMessage.create({
        ...
        content: `
            <div style="background: linear-gradient(to bottom, #001a33, #003366); border: 2px solid orange; padding: 15px; color: #b3d9ff;">
                <h2 style="color: orange; text-align: center; font-family: 'Orbitron'; text-shadow: 0 0 5px #00ccff;">Scar</h2>
                <div style="text-align: center; margin-bottom: 10px; font-size: 0.8em; color: #66a3ff;">Isaac - Marina General</div>
                <p style="border-top: 1px solid #004d99; padding-top: 10px; font-style: italic;">Focar nas feridas abertas.</p>
                <div style="background: #00264d; padding: 5px; text-align: center; margin-top: 10px; border: 1px solid #004d99;">
                    <strong>DANO/EFEITO:</strong> <span style="color: #33ccff;">+1d8 Dmg</span>
                </div>
            </div>
        `
    });
    ...
    // Roll Padrão
    const dmg = "+1d8 Dmg";
    if (dmg && dmg.includes("d") && "Scar" !== "The Six Beasts") {
        await OmniCore.dice.roll(dmg);
    }
