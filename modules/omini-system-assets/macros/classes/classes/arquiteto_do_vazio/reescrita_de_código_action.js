/* ═══════════════════════════════════════════════════════════════
 * CLASS FEATURE: Reescrita de Código
...
(async () => {
    ...
    if ("Reescrita de Código" === "Metavisão de Combate") {
    ...
    ChatMessage.create({
        ...
        content: `
            <div style="${bgStyle} padding: 10px; color: #fff; font-family: 'Courier New', monospace;">
                <h3 style="color: ${borderColor}; text-transform: uppercase; border-bottom: 1px dashed ${borderColor};">Reescrita de Código</h3>
                <div style="font-size: 0.8em; color: #888;">&lt;SYSTEM_OVERRIDE: Arquiteto do Vazio&gt;</div>
                <p style="margin-top: 10px;">Reescreve o tipo de dano recebido (Ex: Fogo -> Cura).</p>
                <div style="background: rgba(0,255,0,0.1); border: 1px solid ${borderColor}; padding: 5px; text-align: center; margin-top: 10px;">
                    <strong>STATUS:</strong> <span style="color: ${borderColor};">Change Element</span>
                </div>
            </div>
        `
    });
