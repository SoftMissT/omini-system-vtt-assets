...
class IncarnationEngine {
  ...
  static async manifest(actor, type, target = null) {
    ...
    if (state.conviction < config.cost) {
      ui.notifications.warn(
        `⚠️ Conviction insuficiente (${state.conviction}/${config.cost})`,
      );
      return null;
    }
  ...
  static async _enhanceArmament(actor) {
    ...
    if (game.modules.get("sequencer")?.active) {
      try {
        const { Sequence } =
          await import("/modules/sequencer/scripts/module.js");
        await new Sequence()
          .effect()
  ...
  static async _releaseRecollection(actor, target) {
    ...
    const confirm = await Dialog.confirm({
      ...
      content:
        "<p>Release Recollection consumirá <strong>50% da Durabilidade</strong> da arma. Se falhar, a arma pode quebrar. Continuar?</p>",
    });
    ...
    const chant = await Dialog.prompt({
      ...
      content:
        "<p>Recite a memória (Ex: 'Floresça, Rosa Azul'):</p><input type='text'/>",
  ...
  static async recoverConviction(actor, amount) {
    ...
    ui.notifications.info(
      `✨ Conviction recuperada: +${amount} (${state.conviction}/100)`,
    );
  }
}
