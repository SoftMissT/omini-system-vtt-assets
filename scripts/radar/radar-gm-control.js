...
class GMControlHUD extends HandlebarsApplicationMixin(ApplicationV2) {
  constructor(options = {}) {
    super(options);
  ...
  async _prepareContext(options) {
    const players = game.users
      ...
      .map((u) => ({ name: u.name, actorId: u.character.id }));
    ...
    this.element
      ...
      .forEach((btn) => {
        btn.addEventListener("click", (e) =>
          this._selectEvent(e.currentTarget.dataset.type),
        );
      });
  }
  ...
  _selectEvent(type) {
    ...
    ui.notifications.info(
      `Modo de posicionamento: ${this.placingEvent.label}. Clique no canvas.`,
    );
  }
  ...
  async _onCanvasClick(event) {
    ...
    const targetId =
      this.element.querySelector('[name="targetActorId"]')?.value || "all";
    const message =
      this.element.querySelector('[name="message"]')?.value ||
      "Sinal Detectado";

    // Use RadarCore if available, otherwise fallback to direct flag setting
    if (window.RadarCore) {
      await RadarCore.placeSignal({
        type: this.placingEvent.key,
        position: pos,
        targetId,
        message,
      });
      ...
      if (targetId === "all") {
        for (const p of game.users.filter((u) => !u.isGM && u.character))
          await p.character.setFlag(FLAG_SCOPE, EVENT_KEY, payload);
      } else {
...
globalThis.GMControlHUD = GMControlHUD;
