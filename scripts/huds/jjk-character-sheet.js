...
export class CharacterSheetJJK extends HandlebarsApplicationMixin(ApplicationV2) {
    ...
    async _prepareContext(options) {
        const data = this.actor.getFlag('world', `sao_jjk_sheet_${this.actor.id}`) || {
            energy: 100,
            maxEnergy: 100,
            technique: 'Nenhuma',
    ...
    _scheduleSave(field, value) {
        ...
        this._saveTimeout = setTimeout(async () => {
            const currentData = this.actor.getFlag('world', `sao_jjk_sheet_${this.actor.id}`) || {};
            foundry.utils.setProperty(currentData, field, value);
            
            await this.actor.setFlag('world', `sao_jjk_sheet_${this.actor.id}`, currentData);
    ...
    async _onToggleHeavenly(event, target) {
        ...
        if (newValue) {
            ui.notifications.warn("Restrição Celestial Ativa: Energia Amaldiçoada zerada.");
        }
    ...
    async _onExpandDomain(event, target) {
        ...
        if (currentData.energy < 50) {
            return ui.notifications.error("Energia Amaldiçoada insuficiente para expansão!");
        }

        // Emitir evento para o Radar
        Hooks.call('omniCore.radar.eventPlaced', {
            type: 'special',
            position: canvas.tokens.get(this.actor.getActiveTokens()[0]?.id)?.center || {x: 0, y:0},
            targetId: 'all',
