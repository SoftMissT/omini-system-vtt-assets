...
const OMINI_CONFIG = {
    ...
    TIERS: {
        ...
        INCOMUM: { color: "#4caf50", glow: "green wisps", frame: "enhanced bronze" },
        RARO: { color: "#42a5f5", glow: "purple crystalline", frame: "ornate gold gemstones" },
        EPICO: { color: "#9c27b0", glow: "crimson lightning arcs", frame: "dragon-scale molten" },
        LENDARIO: { color: "#ffb74d", glow: "golden cosmic rays", frame: "celestial mandala prismatic" },
        MITICO: { color: "#bd00ff", glow: "void tendrils reality cracks", frame: "impossible geometry eldritch" }
    }
};
...
class FusionEngine {
    ...
    async fuseItems(item1, item2, catalyst = null) {
        ...
        let prompt = PROMPT_TEMPLATES.FUSION
            .replace('{ITEM_1}', JSON.stringify(item1, null, 2))
...
class OminiDatabaseGenerator extends HandlebarsApplicationMixin(ApplicationV2) {
    constructor(options = {}) {
        super(options);
    ...
    static PARTS = {
        main: {
            template: "modules/omini-system-vtt-assets/templates/macros/db-generator.hbs"
        }
    };
    ...
    _onRender(context, options) {
        ...
        // Bind Events
        html.querySelector("[name='category']")?.addEventListener("change", (e) => {
        ...
        html.querySelector(".btn-generate")?.addEventListener("click", this._onGeneratePrompts.bind(this));
        html.querySelector(".btn-fuse")?.addEventListener("click", this._onFuseItems.bind(this));
        html.querySelector(".btn-save-config")?.addEventListener("click", this._onSaveConfig.bind(this));
    ...
    async _onGeneratePrompts(event) {
        ...
        try {
            this.state.results = await this.iconSynth.generateLibrary(this.state.category, items, this.state.style);
            this.render();
    ...
    async _createItemInFoundry(itemData) {
        ...
        if (item) {
             ui.notifications.info(`✅ ${itemData.name} criado no diretório de itens!`);
        }
    }
}
...
╚══════════════════════════════════════════════════════════════════════════╝
    ...
    ui.notifications.info("🌌 OMINI Database Generator carregado! Use: OminiDB.open()");
});
